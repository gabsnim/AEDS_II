...
void inserir (pokeref* array, pokeref pokemon, int n, int pos)
{
    for(int i = n; i > pos; i--)
    {
        array[i] = array[i-1];
...
pokeref removerInicio(pokeref* array)
{
    int i;
    pokeref resp;
...
pokeref removerFim (pokeref* array)
{
    return array[--n];
}

pokeref remover (pokeref* array, int pos)
{
    pokeref resp;
    resp = array[pos];
    n--;
