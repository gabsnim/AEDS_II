...
void inserirInicio (pokeref* array,pokeref pokemon, int n)
{
    int i;
...
void inserirFim (pokeref* array, pokeref pokemon, int n)
{
    array[n] = pokemon;
    n++;
}

void inserir (pokeref* array, pokeref pokemon, int pos, int n)
{
    for(int i = n; i > pos; i--)
    {
        array[i] = array[i-1];
...
pokeref removerInicio(pokeref* array, int n)
{
    int i;
    pokeref resp;
...
pokeref removerFim (pokeref* array, int n)
{
    printf("(R) %s\n", array[n-1]->name);
    return array[--n];
}

pokeref remover (pokeref* array, int pos, int n)
{
    pokeref resp;
    resp = array[pos];
...
int main (int argc, char** argv)
{
    ...
    //repetir para quantidade de comandos
    for(int i = 0; i < y; i++)
    {
