...
int findMax (pokeref array[], int n)
{
...
void radcountingSort(pokeref array[], int n, int charIndex)
{
    pokeref *output = malloc(n * sizeof(pokeref));
    int count[256] = {0};


    for(int i = 0; i < n; i++)
    {
        int charValue = (charIndex < strlen(array[i]->abilities[0])) ? array[i]->abilities[0][charIndex] : 0;
        count[charValue]++;
    }
...
void radixSort(pokeref array[], int n)
{
    int maxLen = findMax(array, n);

    for(int i = maxLen - 1; i >= 0; i--)
    {
        radcountingSort(array, n, i);
    }
}
