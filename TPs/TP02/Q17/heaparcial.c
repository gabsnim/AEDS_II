...
void heapify(pokeref array[], int n, int i)
{
    int largest = i;
    int left = 2 * i + 1;
    int right = 2 * i + 2; 
    ...
    else if (left < n && array[left]->height == array[largest]->height)
    {
        if (strcmp(array[left]->name, array[largest]->name) < 0)
        { 
            largest = left;
        }
    }
    ...
    else if (right < n && array[right]->height == array[largest]->height)
    {
        if (strcmp(array[right]->name, array[largest]->name) < 0) 
        { 
            largest = right;
        }
    }
...
void heapSortParcial(pokeref array[], int n) 
{
