...
int findMinIndex (pokeref array[], int left, int right)
{
    int minIndex = left;
    for(int i = left + 1; i <= right; i++)
    {
        if(strcmp(array[i]->name, array[minIndex]->name) < 0)
        {
            minIndex = i;
        }
    }
...
void selectionSortRec(pokeref array[], int left, int right)
{
    if(left >= right)
    {
        return array;
    ...
    int minIndex = findMinIndex(array, left, right);
    ...
    selectionSortRec(array, left+1, right);
}

void selectionRec(pokeref array[], int n)
{
    selectionSortRec(array, 0, n-1);
}
