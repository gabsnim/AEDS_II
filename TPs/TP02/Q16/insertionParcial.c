...
int compareDate(char* dateX, char* dateY)
{
    int dayX, monthX, yearX;
    int dayY, monthY, yearY;

    sscanf(dateX, "%d/%d/%d", &dayX, &monthX, &yearX);
    sscanf(dateY, "%d/%d/%d", &dayY, &monthY, &yearY);
...
void insertionParcial (pokeref array[], int n)
{
    for(int i = 1; i < n; i++)
    {
        pokeref key = array[i];
        ...
        while (j >= 0 && compareDate(array[j]->captureDate, key->captureDate) > 0)
        {
            array[j+1] = array[j];
            j--;
        }
