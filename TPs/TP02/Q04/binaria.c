...
void selectionSort (pokeref array[], int n)
{
    for (int i = 0; i < n - 1; i++)
    {
        int lowest = i;
        for (int j = i + 1; j < n; j++)
        {
            if (strcmp(array[j]->name, array[lowest]->name) < 0)
            {
                lowest = j;
            }
        }
...
void binarySearch (pokeref array [], int n, char *name)
{
    int left = 0;
    int right = n - 1;
    int found = false;
...
void log(double executionTime, int comp, int movs)
{
    FILE* arquivo = fopen("805347_binaria.txt", "wt");

    fprintf(arquivo, "%s\t%.6f\t%d\t%d\n", "805347", executionTime, comp, movs);
    fclose(arquivo);
}
